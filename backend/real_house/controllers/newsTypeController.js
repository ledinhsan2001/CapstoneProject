...
exports.getAll = catchAsync(async(req, res) => {
    ...
    {
        throw new ApiError(400, "Danh sách kiểu tin trống.");
    }
})
...
exports.create = catchAsync(async(req,res) => {
    ...
    if(!name_exist){
        if(!id_exist){
            const news_type = await new NewsType({_id:id, name,  unit_price}).save();
            if(news_type) {
...
exports.put = catchAsync(async(req, res) => {
    const { id, name, unit_price } = req.body;
    const news_type = await NewsType.findByIdAndUpdate(id,{name, unit_price}, {new:true});
    if(news_type){
        news_type.__v = undefined;
