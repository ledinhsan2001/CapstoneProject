...
exports.getAll = catchAsync(async(req, res) => {
    ...
    {
        throw new ApiError(400, "Danh sách số ngày trống.");
    }
})
...
exports.create = catchAsync(async(req,res) => {
    ...
    if(!data){
        if(!id_exist){
            const data = await new NumberDay({_id:id, number_day, saved_price}).save();
            if(data) {
                data.__v = undefined;
                res.status(201).json({
                    success:true,
...
exports.put = catchAsync(async(req, res) => {
    const { id, number_day, saved_price } = req.body;
    const data = await NumberDay.findByIdAndUpdate(id,{number_day, saved_price}, {new:true});
    if(data){
        data.__v = undefined;
